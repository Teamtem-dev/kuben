...
| `.github/workflows/site.yml` | Builds `apps/site` (kuben.teamtem.com) on pull requests that touch it; deploys it to Cloudflare Pages on pushes to `main`. Deploying needs the variable `CLOUDFLARE_ACCOUNT_ID` and the secret `CLOUDFLARE_API_TOKEN`; without the variable the deploy job is skipped. |
