...
func endpointIdentity(org ids.OrgID, endpoint uuid.UUID) secrets.Identity {
	return secrets.Identity{Org: org.String(), Secret: endpoint.String(), Revision: 0}
}

// SealSecret seals a new endpoint secret for endpoint of org (seal_secret).
func SealSecret(keyring *secrets.Keyring, org ids.OrgID, endpoint uuid.UUID, secret []byte) (store.SealedBytes, error) {
...
func OpenSecret(keyring *secrets.Keyring, org ids.OrgID, endpoint uuid.UUID, sealed store.SealedBytes) ([]byte, error) {
