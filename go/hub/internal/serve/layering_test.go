...
// module is the import path of the hub module; its packages live under the
// module root, two directories above this one.
const module = "github.com/Teamtem-dev/kuben/go/hub/"

// TestTheServerDoesNotDependOnTheCLI keeps the server free of internal/cli:
// what both need lives in internal/ops, internal/install and the like, and
	...
	for len(queue) > 0 {
		...
		for _, imp := range pkg.Imports {
			...
			if strings.HasPrefix(imp, module+"internal/cli") {
				t.Errorf("internal/serve depends on %s (imported by %s)", imp, path)
				continue
			}
