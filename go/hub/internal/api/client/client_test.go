...
func TestAppPathsTakeDefaultsAndRefuseOddNames(t *testing.T) {
	full, err := client.ParseAppPath("shop/prod/web", none(), none())
	if err != nil {
		t.Fatal(err)
	}
	...
	defaults, err := client.ParseAppPath("web", some("shop"), some("dev"))
	if err != nil {
		t.Fatal(err)
	}
	if diff := cmp.Diff(client.AppPath{Project: "shop", Environment: "dev", App: "web"}, defaults); diff != "" {
		t.Errorf("(-want +got):\n%s", diff)
	}
	given, err := client.ParseAppPath("live/web", some("shop"), some("dev"))
	...
	for _, refused := range []struct {
		...
		if _, err := client.ParseAppPath(refused.text, refused.project, refused.environment); err == nil {
...
func TestTheTokenNeverShowsInDebugOutput(t *testing.T) {
	c := client.New("https://kuben.example.com/", "kbn_secret")
...
func TestLogQueriesCarryOnlyWhatIsAsked(t *testing.T) {
	if got := (client.LogOptions{}).Query(false); got != "" {
		t.Errorf("default: %q", got)
	}
	options := client.LogOptions{Tail: opt.Some[int64](20), Process: some("worker"), Previous: true}
	if got := options.Query(true); got != "?tail=20&process=worker&previous=true&follow=true" {
		t.Errorf("all: %q", got)
	}
	odd := client.LogOptions{Process: some("a&b")}
...
func TestServerSentEventsAreSplitAcrossChunks(t *testing.T) {
	var parser client.SSEParser
	if got := parser.Push("event: line\ndata: {\"a\""); len(got) != 0 {
		t.Errorf("incomplete: %v", got)
	}
	got := parser.Push(":1}\n\n: ping\n\nevent: end\r\ndata: {}\r\n\r\ndata: x\n")
	want := []client.SSEEvent{{Event: "line", Data: `{"a":1}`}, {Event: "end", Data: "{}"}}
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("(-want +got):\n%s", diff)
	}
	if diff := cmp.Diff([]client.SSEEvent{{Event: "message", Data: "x"}}, parser.Push("\n")); diff != "" {
		t.Errorf("(-want +got):\n%s", diff)
	}
}
...
func serve(t *testing.T, handler http.HandlerFunc) client.Client {
	...
	return client.NewWith(srv.URL+"/", "kbn_x", srv.Client().Transport)
}
...
func TestProblemsBecomeAPIErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want:   client.APIError{Status: 404, Code: "not_found", Detail: some("no app web")},
		...
		},
		{
			...
			want:   client.APIError{Status: 403, Code: "forbidden"},
			shown:  "403 (forbidden)",
		},
		{
			...
			want:   client.APIError{Status: 502, Code: "502"},
			shown:  "502 (502)",
		},
	}
...
func TestFollowedLogsArriveAsEvents(t *testing.T) {
	...
	events, err := c.FollowLogs(t.Context(), app, client.LogOptions{Tail: opt.Some[int64](5)})
	if err != nil {
		t.Fatal(err)
	}
	var got []client.FollowEvent
	...
	want := []client.FollowEvent{
		client.LineEvent{LogLine: client.LogLine{Pod: "web-1", Process: some("web"), Line: "héllo"}},
		client.EndEvent{},
	}
