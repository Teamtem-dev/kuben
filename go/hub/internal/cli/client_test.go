...
func testRelease(revision int64, current bool) client.ReleaseDto {
	return client.ReleaseDto{Revision: revision, Reason: "deploy", Current: current}
}

func TestRevisionsAreFoundInTheHistory(t *testing.T) {
	history := []client.ReleaseDto{testRelease(5, false), testRelease(4, true), testRelease(3, false), testRelease(1, false)}
	...
	if previousRevision([]client.ReleaseDto{testRelease(1, true)}).IsSome() {
...
func TestAppsArePrintedAsATable(t *testing.T) {
	rows := []client.AppDto{
		{
			...
			Processes: []client.ProcessDto{{Name: "web"}, {Name: "worker"}},
		},
	...
	out.Reset()
	if err := printApps(&out, []client.AppDto{}, true); err != nil {
		t.Fatal(err)
	}
	if out.String() != "[]\n" {
...
func statusFixture() (client.AppPath, client.AppDetail, []client.ReleaseDto, client.DoctorReport) {
	path := client.AppPath{Project: "shop", Environment: "prod", App: "web"}
	detail := client.AppDetail{
		App: client.AppDto{
...
`
	...
	detail.Pods = nil
	forbidden := client.APIError{Status: 403, Code: "forbidden"}
...
func TestAppStatusJSONHasSortedKeys(t *testing.T) {
	detail := client.AppDetail{App: client.AppDto{
		Name: "web", Project: "shop", Environment: "prod", Namespace: "shop-prod", Ready: true,
		Processes: []client.ProcessDto{}, Env: []client.EnvVarDto{}, Domains: []string{}, Volumes: []client.VolumeDto{},
	}}
	releases := []client.ReleaseDto{{Revision: 1, Reason: "create", CreatedAt: 1700000000000, Current: true}}
	doctor := client.DoctorReport{
		Status: "ok", Checks: []client.DoctorCheck{},
...
`
	...
	out.Reset()
	if err := printStatusJSON(&out, detail, releases, opt.None[client.DoctorReport]()); err != nil {
		t.Fatal(err)
	}
...
func TestFollowedLogsStopAtTheEndOfTheStream(t *testing.T) {
	events := []client.FollowEvent{
		client.LineEvent{LogLine: client.LogLine{Pod: "web-1", Line: "2026-09-25T10:00:00Z kept as sent"}},
		client.EndEvent{LogEnd: client.LogEnd{Pod: opt.Some("web-1"), Error: opt.Some("container exited")}},
		client.EndEvent{LogEnd: client.LogEnd{Pod: opt.Some("web-2")}},
		client.EndEvent{},
		client.LineEvent{LogLine: client.LogLine{Pod: "web-3", Line: "never printed"}},
	}
	seq := func(yield func(client.FollowEvent, error) bool) {
	...
	broken := func(yield func(client.FollowEvent, error) bool) {
		yield(nil, client.TransportError{Reason: "unexpected event: EOF"})
	}
...
func TestDeploymentsAreWaitedFor(t *testing.T) {
	app := client.AppPath{Project: "shop", Environment: "prod", App: "web"}
	tests := []struct {
		name    string
		start   string
		phases  []string
