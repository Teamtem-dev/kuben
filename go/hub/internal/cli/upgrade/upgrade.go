// Package upgrade is upgrades (crates/kuben/src/cli/upgrade.rs, M4.8):
// `kuben upgrade-check`, run with the *new* binary before it replaces the
// old one, and the journaled migration every start and `kuben migrate` go
// through, with a backup first when migrations are pending.
package upgrade
...
func Check(ctx context.Context, cfg config.Config, major bool, out io.Writer) error {
	...
	findings := coreupgrade.Preflight(f)
	failed := false
