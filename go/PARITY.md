...
| kuben | `cli/backup.rs` | 561 | 5 | ops/backup, cli (cmd_backup.go) | ported | 5 → 6 (+ manifest golden); Run/RunTo/Restore/ClientToolsInstalled/Freshness serve ops/upgrade, cli and serve (watch_backups in serve/maintenance.go); paths joined as Path::join did |
...
| kuben | `cli/upgrade.rs` | 134 | 1 | ops/upgrade, cli/upgrade (Check), cli (cmd_upgrade.go), serve | ported | 1 → 1; serve and `kuben migrate` migrate through ops/upgrade.Migrate (a backup first when migrations are pending and pg_dump is installed) |
...
| kuben-api | `host.rs` | 73 | 2 | platform/host | ported | 2 → 2; the only copy: serve, api, bootstrap and cli call it (AdvertiseIP dials udp4, as the Rust bound 0.0.0.0) |
| kuben-api | `image_watch.rs` | 287 | 0 | imagewatch, serve (background.go) | ported | 0 in Rust; covered by the tests/http.rs port below; runs on every replica as serve.rs spawn_background |
...
| kuben-api | `routes/git.rs` | 298 | 0 | api (git.go) | ported | 0 → 3 (webhook refusals/outcomes, mounting and the 5 MiB limit, provider errors); mounted beside the CI exchange (inside the gzip/request-id/recovery wrappers; an oversized body is an empty 413); pull requests go to Previews.OnPull |
...
| kuben-api | `routes/previews.rs` | 325 | 0 | api (previews_routes.go) | ported | get/put policy, list, extend, destroy; put answers the updatedAt it stored (one clock reading) |
