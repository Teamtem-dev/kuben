...
Templates live in `internal/httpapi/templates.go` and follow one rule: **no fixed passwords**, ever. Open a pull request with the image pinned to a major tag, the volume paths the image documents, and the keys that must be generated.
