...
| `KUBEN_BOOTSTRAP__ADMIN_PASSWORD` | generated | a generated password is stored in the Secret `kuben-initial-admin` (in Kubernetes); the binary prints it once to its terminal or, without one, writes it to `initial-admin-password` next to the database (owner-only). Never logged |
