...
- The image is built from the very binaries that were checksummed and attested, and scanned for HIGH and CRITICAL vulnerabilities before it is pushed. Every binary embeds its dependency list (Go's module list, `go version -m`; 1.x: `cargo auditable`), so `trivy`, `grype`, `osv-scanner` or `govulncheck -mode=binary` can audit a deployed binary in place.

## Supply chain

- Every GitHub Action is pinned to a commit SHA; workflows are audited with zizmor on every pull request.
- Releases are signed keyless with Sigstore (checksums, image, chart) and carry build provenance; the installer verifies the signature when cosign is present. See [Download, verify, then run](/docs/getting-started/binary/#download-verify-then-run).
- What setup installs from others (k3s, Gateway API CRDs, cert-manager and its images, PostgreSQL) is pinned by digest in the release's bundle lock and checked before use.
