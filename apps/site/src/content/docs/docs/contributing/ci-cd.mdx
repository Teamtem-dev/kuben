...
| **Store matrix** | the store tests against PostgreSQL 17, the only database Kuben keeps its data in |
...
| **Binary size budget** | a static musl release build with the embedded console and dependency list, gated at 30 MiB, then a Trivy scan of the binary |
...
plus a `.sha256` per archive, `checksums.txt` and its keyless Sigstore signature `checksums.txt.sigstore.json`, `install.sh`, `bundle.lock.json` (what setup installs besides Kuben, pinned), the image `ghcr.io/teamtem-dev/kuben:{X.Y.Z, X.Y, latest}` and the chart `oci://ghcr.io/teamtem-dev/charts/kuben`. Tags like `v1.1.0-rc.1` become pre-releases and never move `latest`.
