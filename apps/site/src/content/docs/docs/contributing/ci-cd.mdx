...
| **Binary size budget** | a static musl release build with the embedded console and dependency list, gated at 28 MiB, then a Trivy scan of the binary |
...
tag v1.0.2 ─► plan (tag == Cargo version)
               ...
               │                                                                     └─► image (budget ≤ 32 MiB → Trivy → push amd64+arm64, SBOM, provenance)
