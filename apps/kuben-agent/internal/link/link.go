...
//   - Execution envelopes ([agentlink.Apply]) go to the [Executor], one task
...
func NewCredentials(pinned []*x509.Certificate, hubName string, identity tls.Certificate, lifetime Lifetime) (*Credentials, error) {
	cfg, err := agentlink.AgentConfig(pinned, hubName, &identity)
	if err != nil {
...
func (c *Credentials) Replace(identity tls.Certificate, lifetime Lifetime) error {
	cfg, err := agentlink.AgentConfig(c.pinned, c.hubName, &identity)
	if err != nil {
...
type Executor interface {
	...
	Apply(ctx context.Context, apply agentlink.Apply, reports chan<- agentlink.Observation)
}
...
type Config struct {
	ClusterID    string
	AgentVersion string
	Capabilities agentlink.Features
...
func NewConfig(clusterID, agentVersion string, credentials *Credentials, logger *slog.Logger) Config {
	return Config{
		ClusterID: clusterID, AgentVersion: agentVersion, Capabilities: agentlink.Features{}, Credentials: credentials,
...
func IsLasting(err error) bool {
	...
	case agentlink.RefusalRevoked, agentlink.RefusalUnknownCluster, agentlink.RefusalUnsupportedProtocol:
		return true
	case agentlink.RefusalUnsupportedCapability, agentlink.RefusalBadRequest, agentlink.RefusalInvalidToken, agentlink.RefusalOther:
	}
	return false
}
...
type Session struct {
	Negotiated agentlink.Negotiated
	HubVersion string
...
func Handshake(conn net.Conn, cfg Config) (Session, error) {
	hello := agentlink.Hello{
		ProtocolVersions: agentlink.SupportedVersions.Descending(),
	...
	if err := agentlink.WriteFrame(conn, hello); err != nil {
	...
	switch m := m.(type) {
	case agentlink.Welcome:
		if !agentlink.SupportedVersions.Contains(m.ProtocolVersion) {
		...
		return Session{
			Negotiated: agentlink.Negotiated{Version: m.ProtocolVersion, Features: m.Features},
			HubVersion: m.HubVersion,
			Heartbeat:  max(heartbeat, minHeartbeat),
		}, nil
	case agentlink.Refused:
		return Session{}, RefusedError(m)
	case agentlink.Hello, agentlink.Heartbeat, agentlink.HeartbeatAck, agentlink.Enroll, agentlink.Renew,
		agentlink.Enrolled, agentlink.Apply, agentlink.Observation, agentlink.Unknown:
	}
