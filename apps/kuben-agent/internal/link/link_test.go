...
// the hub is internal/agentlink/link_test.go (tests/link.rs).
...
func TestOnlyRefusalsTheHubMustLiftAreLasting(t *testing.T) {
	refused := func(reason agentlink.Refusal) error { return link.RefusedError{Reason: reason} }
	if !link.IsLasting(refused(agentlink.RefusalRevoked)) || !link.IsLasting(refused(agentlink.RefusalUnsupportedProtocol)) ||
		!link.IsLasting(refused(agentlink.RefusalUnknownCluster)) {
		t.Fatal("lasting refusals")
	}
	if link.IsLasting(refused(agentlink.RefusalBadRequest)) || link.IsLasting(link.ErrClosed) {
