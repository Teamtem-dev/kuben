...
import (
	...
	"github.com/Teamtem-dev/kuben/apps/kuben-agent/internal/clock"
...
func (e *KubeExecutor) writeRuntime(ctx context.Context, apply agentlink.Apply, spec v1alpha1.ApplicationRuntimeSpec) (*unstructured.Unstructured, error) {
...
func (e *KubeExecutor) applyResources(ctx context.Context, apply agentlink.Apply, checked Checked, runtime *unstructured.Unstructured) error {
...
func (e *KubeExecutor) prune(ctx context.Context, apply agentlink.Apply, checked Checked, ownerUID string) error {
	keep := checked.keep()
...
// passes.
func (e *KubeExecutor) verify(ctx context.Context, apply agentlink.Apply, checked Checked) Rollout {
...
func (e *KubeExecutor) writeStatus(ctx context.Context, apply agentlink.Apply, checked Checked, ready bool, reason, message string) {
...
func (e *KubeExecutor) Apply(ctx context.Context, apply agentlink.Apply, reports chan<- agentlink.Observation) {
	e.Carry(ctx, apply, reports)
}

// Carry carries apply out, reporting each step.
func (e *KubeExecutor) Carry(ctx context.Context, apply agentlink.Apply, reports chan<- agentlink.Observation) {
	// observe reports a phase; with a reason, the message goes along.
	observe := func(generation int64, phase agentlink.RuntimePhase, reason, message string) {
		o := agentlink.Observation{Target: apply.Target, Generation: generation, Phase: phase}
		if reason != "" {
	...
	if err != nil {
		...
		observe(0, agentlink.RuntimePhaseRejected, refused.Reason, refused.Message)
		return
	}
	generation := checked.Spec.Generation
	runtime, err := e.writeRuntime(ctx, apply, checked.Spec)
	if err != nil {
		phase, reason := agentlink.RuntimePhaseFailed, "KubernetesError"
		if isStatus(err, 422) {
			phase, reason = agentlink.RuntimePhaseRejected, "EnvelopeRefused"
		}
		observe(generation, phase, reason, err.Error())
		return
	}
	observe(generation, agentlink.RuntimePhaseAccepted, "", "")
	if err := e.applyResources(ctx, apply, checked, runtime); err != nil {
		...
		observe(generation, agentlink.RuntimePhaseFailed, refused.Reason, refused.Message)
		return
	}
	...
	observe(generation, agentlink.RuntimePhaseApplying, "", "")
	...
	case RolloutReady:
		...
		observe(generation, agentlink.RuntimePhaseReady, "", "")
	case RolloutFailed:
...
func (e *KubeExecutor) rolloutFailed(ctx context.Context, apply agentlink.Apply, checked Checked, message string,
	observe func(int64, agentlink.RuntimePhase, string, string),
) {
	e.writeStatus(ctx, apply, checked, false, "RolloutFailed", message)
	observe(checked.Spec.Generation, agentlink.RuntimePhaseFailed, "RolloutFailed", message)
}
