...
func TestAPlanThatDoesNotMatchItsDigestIsRefused(t *testing.T) {
	if r := refusal(t, applyOf(t, `[`+deployment+`]`, agentlink.HexDigest([]byte("other")))); r != "DigestMismatch" {
		t.Fatal(r)
	}
}
...
func TestOnlyAllowedKindsInTheEnvelopesNamespaceAreApplied(t *testing.T) {
	...
	missing := agentlink.Apply{Namespace: "kb-shop-prod", Spec: `{"targetId":"t"}`}
