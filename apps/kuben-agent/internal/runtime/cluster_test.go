...
func (n testNS) envelope(t *testing.T, generation int64, resources []any, digest string) agentlink.Apply {
	t.Helper()
	...
	if digest == "" {
		digest = agentlink.HexDigest(text)
	}
	spec, err := json.Marshal(v1alpha1.ApplicationRuntimeSpec{
		...
		Generation: generation, InputHash: agentlink.HexDigest(fmt.Appendf(nil, "input-%d", generation)),
	...
	return agentlink.Apply{Target: "0199a0c0-0000-7000-8000-000000000001", Namespace: n.name, Name: "web", Spec: string(spec)}
}

// carry carries apply out and returns the observations, in order.
func carry(t *testing.T, e *runtime.KubeExecutor, apply agentlink.Apply) []agentlink.Observation {
	t.Helper()
	reports := make(chan agentlink.Observation, 16)
	...
	close(reports)
	var seen []agentlink.Observation
	for o := range reports {
		seen = append(seen, o)
	}
	return seen
...
func phases(seen []agentlink.Observation) []agentlink.RuntimePhase {
	out := make([]agentlink.RuntimePhase, 0, len(seen))
	for _, o := range seen {
...
func lastPhase(seen []agentlink.Observation) agentlink.RuntimePhase {
	if len(seen) == 0 {
		return ""
	}
...
func TestTheAgentCarriesAnEnvelopeOutAndPrunesWhatANewerPlanDropped(t *testing.T) {
	...
	want := []agentlink.RuntimePhase{agentlink.RuntimePhaseAccepted, agentlink.RuntimePhaseApplying, agentlink.RuntimePhaseReady}
	...
	if lastPhase(second) != agentlink.RuntimePhaseReady {
		t.Fatalf("%+v", second)
	}
	gone := false
	for range 30 {
	...
	if !slices.Equal(phases(stale), []agentlink.RuntimePhase{agentlink.RuntimePhaseRejected}) ||
...
func TestTheSameEnvelopeCarriedAgainChangesNothing(t *testing.T) {
	...
	if first := carry(t, e, envelope); lastPhase(first) != agentlink.RuntimePhaseReady {
		t.Fatalf("%+v", first)
	}
	...
	if lastPhase(again) != agentlink.RuntimePhaseReady || slices.Contains(phases(again), agentlink.RuntimePhaseRejected) {
...
func TestTheAgentRefusesAWrongDigestOrKindBeforeWritingAnything(t *testing.T) {
	...
	forged := carry(t, e, ns.envelope(t, 1, []any{ns.deployment("web-web")}, agentlink.HexDigest([]byte("something else"))))
