...
func (c testCA) issue(t *testing.T, key agentlink.DeviceKey, now time.Time, lifetime time.Duration) string {
	t.Helper()
	tmpl := &x509.Certificate{
	...
	return agentlink.EncodePEM("CERTIFICATE", der)
}
...
func TestThePinnedCAFileMustHoldACertificate(t *testing.T) {
	...
	if err := os.WriteFile(file, []byte(agentlink.EncodePEM("CERTIFICATE", ca.cert.Raw)), 0o600); err != nil {
		t.Fatal(err)
	}
