...
func ParseStoredCertificate(text string) (StoredCertificate, error) {
	cert, err := agentlink.ParseCertificatePEM(text)
	if err != nil {
...
func (s State) DeviceKey() (agentlink.DeviceKey, error) {
	path := s.path(DeviceKey)
	text, err := os.ReadFile(path) //nolint:gosec // the agent's own path
	switch {
	case err == nil:
		key, err := agentlink.ParseDeviceKey(string(text))
		if err != nil {
			return agentlink.DeviceKey{}, InvalidError{Path: path, Reason: err.Error()}
		}
		return key, nil
	case !errors.Is(err, fs.ErrNotExist):
		return agentlink.DeviceKey{}, IOError{Path: path, Err: err}
	}
	key, err := agentlink.GenerateDeviceKey()
	if err != nil {
		return agentlink.DeviceKey{}, err //nolint:wrapcheck // says what failed
	}
	pem, err := key.PEM()
	if err != nil {
		return agentlink.DeviceKey{}, err //nolint:wrapcheck // says what failed
...
func PinnedCA(path string) ([]*x509.Certificate, error) {
	...
	cas, err := agentlink.ParseCertificatesPEM(text)
	if err != nil {
...
func EnsureIdentity(ctx context.Context, s State, key agentlink.DeviceKey, hub HubAddress, clusterID string,
	...
	cfg, err := agentlink.AgentConfig(hub.Pinned, hub.Name, nil)
	if err != nil {
	...
	enrolled, err := agentlink.RequestEnrollment(conn, secret, clusterID, key)
	if err != nil {
		return tls.Certificate{}, err //nolint:wrapcheck // says what failed
	}
