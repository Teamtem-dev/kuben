...
// expires.
//
// The agent is a module of its own so that it stays small: it imports only
// its own packages and packages/api, never the hub, the store or
// controller-runtime (ADR-027).
package main
...
import (
	...
	"github.com/Teamtem-dev/kuben/apps/kuben-agent/internal/bootstrap"
...
func agent(ctx context.Context, a args, logger *slog.Logger) error {
	...
	hubName, err := agentlink.ServerName(a.hubName)
	if err != nil {
		return err //nolint:wrapcheck // says what failed
	}
