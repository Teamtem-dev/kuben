...
- Three Go modules, tied together by `go.work` at the repository root:
  `apps/kuben` is the hub (the `kuben` binary, `cmd/kuben`), `apps/kuben-agent`
  the cluster agent (`cmd/kuben-agent`), and `packages/api` what both share:
  the CRD types (`v1alpha1`) and the AgentLink protocol (`agentlink`). Each
  module has exactly one main package; everything not meant for import lives
  under its `internal/`. `ARCHITECTURE.md` lists every package and the
...
- **Exception: `packages/api/v1alpha1`** follows the Kubernetes API conventions
  ...
  controller-gen and client-go are built on them. Code outside v1alpha1
...
Tool versions are pinned in `tools/go.mod`, a module of its own (not a
member of `go.work`) so that their dependencies never reach the binaries:
run them from any directory with `scripts/go-tool.sh <name>`. golangci-lint
is the exception: its authors advise against building it from source, so CI
installs the official release at a pinned version, and `scripts/go-check.sh`
runs it with `.golangci.yml` when it is on the `PATH`.

```bash
bun turbo run lint test --filter=kuben   # one module: go-check.sh, go test -race -shuffle=on
bash scripts/go-check.sh                  # every module: gofumpt, vet, exhaustive, go-check-sumtype, NilAway, golangci-lint
cd apps/kuben && go test -race -shuffle=on ./...
cd apps/kuben && ../../scripts/go-tool.sh govulncheck ./...
```
...
row in `docs/history/SUBSTITUTIONS.md` when it replaces Rust behaviour: do not run
