...
// ends share; the hub's are in apps/kuben/internal/agentlink).
...
func TestADeviceKeySurvivesItsPEMAndKeepsItsID(t *testing.T) {
	...
	back, err := agentlink.ParseDeviceKey(text)
	if err != nil {
		t.Fatal(err)
	}
	...
	token := agentlink.Enroll{Token: agentlink.NewToken("kbt_secret"), ClusterID: "primary"}
	if strings.Contains(fmt.Sprintf("%#v", token), "kbt_secret") {
		t.Fatal("a token never shows in Debug")
	}
	if agentlink.DeviceIDOf(key.PublicKeyInfo()) != key.DeviceID() {
...
func TestTheDeviceKeyAndItsCSRHaveTheShapeRcgenGaveThem(t *testing.T) {
	...
	subject, err := agentlink.UTF8Subject("cluster:primary")
	if err != nil {
		t.Fatal(err)
	}
	...
	csr, err := agentlink.ParseCSR(csrPEM)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := agentlink.ParseCSR("not a csr"); err == nil {
		t.Fatal("garbage parsed")
	}
	// A CSR whose signature does not verify is refused.
	tampered := append([]byte(nil), block.Bytes...)
	tampered[len(tampered)-1] ^= 0xff
	if _, err := agentlink.ParseCSR(agentlink.EncodePEM("CERTIFICATE REQUEST", tampered)); err == nil {
