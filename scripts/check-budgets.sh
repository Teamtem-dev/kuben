...
#
#   scripts/check-budgets.sh binary <path> [max MiB]   default: $KUBEN_BUDGET_BINARY_MB or 28
#   scripts/check-budgets.sh image  <ref>  [max MiB]   default: $KUBEN_BUDGET_IMAGE_MB  or 32
#
...
binary)
  ...
  report "binary $(basename "$target")" "$(wc -c <"$target" | tr -d ' ')" "${3:-${KUBEN_BUDGET_BINARY_MB:-28}}"
  ;;
image)
  bytes=$(docker image inspect --format '{{.Size}}' "$target")
  report "image ${target}" "$bytes" "${3:-${KUBEN_BUDGET_IMAGE_MB:-32}}"
  ;;
*)
