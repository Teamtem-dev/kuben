...
#   KUBEN_BIN=... scripts/e2e.sh     # default: apps/kuben/bin/kuben (bun turbo run build --filter=kuben)
#   KUBEN_AGENT_BIN=...              # default: apps/kuben-agent/bin/kuben-agent (built if missing)
#
...
# Runs from any directory (`turbo run e2e` starts it in apps/kuben).
ROOT=$(cd "$(dirname "$0")/.." && pwd)
BIN=${KUBEN_BIN:-$ROOT/apps/kuben/bin/kuben}
AGENT_BIN=${KUBEN_AGENT_BIN:-$ROOT/apps/kuben-agent/bin/kuben-agent}
...
[[ -x $BIN ]] || { echo "build the binary first: bun turbo run build --filter=kuben ($BIN)" >&2; exit 2; }
...
if [[ -n $AGENT_IMAGE ]]; then
  ...
  if [[ ! -x $AGENT_BIN ]]; then
    ...
    (cd "$ROOT/apps/kuben-agent" && CGO_ENABLED=0 go build -trimpath -o "$AGENT_BIN" ./cmd/kuben-agent)
  fi
