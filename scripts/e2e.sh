...
#   KUBEN_BIN=... scripts/e2e.sh     # default: bin/kuben (bun turbo run go:build)
#   KUBEN_AGENT_BIN=...              # default: bin/kuben-agent (built if missing)
#
...
# Runs from any directory (`turbo run go:e2e` starts it at the root).
ROOT=$(cd "$(dirname "$0")/.." && pwd)
BIN=${KUBEN_BIN:-$ROOT/bin/kuben}
AGENT_BIN=${KUBEN_AGENT_BIN:-$ROOT/bin/kuben-agent}
...
[[ -x $BIN ]] || { echo "build the binary first: bun turbo run go:build ($BIN)" >&2; exit 2; }
...
if [[ -n $AGENT_IMAGE ]]; then
  ...
  if [[ ! -x $AGENT_BIN ]]; then
    ...
    (cd "$ROOT" && CGO_ENABLED=0 go build -trimpath -o "$AGENT_BIN" ./cmd/kuben-agent)
  fi
